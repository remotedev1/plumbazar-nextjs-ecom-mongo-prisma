...
export async function PATCH(req, { params }) {
  try {
    ...
    const { description, discountPercentage, validFrom, validUntil, type, productIds } = body;
...
// DELETE request to soft delete an offer by ID
export async function DELETE(req, { params }) {
  try {
    const offerId = params.id;

    // Soft delete the offer
    const deletedOffer = await db.offer.update({
      where: { id: offerId },
      data: {
