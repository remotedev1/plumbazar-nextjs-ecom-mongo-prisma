...
export function calculateDiscountAndGST(data) {
  // Ensure offers array is defined and has offers
  if (!data.offers || data.offers.length === 0) {
    return {
      offerId: null,
      discountPercentage: 0,
      discountAmount: data.msp, // Return MSP if no discount
      gstAmount: (data.msp * (data.gst ?? 0)) / 100, // Calculate GST on MSP
    };
  }

  const currentDate = new Date();

  // Find valid offers based on the current date within the valid range
  ...
  // If no valid offers, return the default MSP and GST calculations
  if (validOffers.length === 0) {
    return {
      offerId: null,
      discountPercentage: 0,
      discountAmount: data.msp,
      gstAmount: (data.msp * (data.gst ?? 0)) / 100, // GST on MSP
    };
  }

  // Find the highest discount percentage among the valid offers
  const highestOffer = validOffers.reduce((maxOffer, offer) => {
  ...
  const discountAmount = Math.ceil(data.mrp - (data.mrp * highestDiscountPercentage) / 100);

  // Calculate GST on MSP (after discount)
  const gstAmount = (discountAmount * (data.gst ?? 0)) / 100;

  return {
    offerId: highestOffer.id, // Add offerId from the highest offer
