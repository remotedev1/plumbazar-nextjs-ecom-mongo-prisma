...
export const formatPriceToString = (price) => {
  ...
  // Convert integer part to words with a capitalized first letter
  const integerPartInWords = numberToWords
    .toWords(integerPart)
    .replace(/^\w/, (c) => c.toUpperCase());

  // Format the result with "INR"
...
export function calculateDiscountAndGST(data) {
  ...
  const validOffers = offers.filter(({ validFrom, validUntil, deletedAt }) => {
    ...
    return (
      currentDate >= from && currentDate <= until && !deletedAt // Check if deletedAt is not present
    );
  });
